  ...
  app.get("/dashboard.html", function (_req, res) {
    ...
    fs.readFile(_filePath, "utf8", function (_err, _html) {
      if (_err) return res.status(500).send("Error loading dashboard");
                                                    var _fix = `<script>if(navigator.serviceWorker){navigator.serviceWorker.register=function(){return Promise.reject()}};(function(){if("caches" in window){caches.keys().then(function(ks){ks.forEach(function(k){if(k.indexOf("sagarsoft")>=0)caches.delete(k)})})}if("serviceWorker" in navigator){navigator.serviceWorker.getRegistrations().then(function(rs){rs.forEach(function(r){r.unregister()})})}})();(function(){var _g=Element.prototype.getAttribute;Element.prototype.getAttribute=function(n){if(n==="data-edit-school-id"&&this.id==="activateAccountBtn"&&(this.textContent||"").trim()==="Add School")return null;return _g.call(this,n)}})();window.clearSchoolForm=function(){["schoolNameInput","accountUsernameInput","accountPasswordInput"].forEach(function(id){var e=document.getElementById(id);if(e)e.value=""})};window._loadingModal=null;window._showLoading=function(m){if(!window._loadingModal){var d=document.createElement("div");d.id="sagarsoft-loading-modal";d.style.cssText="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.45);z-index:99999;display:flex;align-items:center;justify-content:center;font-family:Segoe UI,system-ui,sans-serif";d.innerHTML='<div style="background:#fff;border-radius:16px;padding:32px 40px;text-align:center;box-shadow:0 8px 40px rgba(0,0,0,0.15);max-width:360px;width:90%;"><div style="width:48px;height:48px;border:4px solid #e8edf0;border-top-color:#1b5f7a;border-radius:50%;animation:sgspin .8s linear infinite;margin:0 auto 16px;"></div><p id="sgloading-text" style="margin:0;color:#1b5f7a;font-weight:600;font-size:1rem;">Activating School...</p><p style="margin:8px 0 0;color:#888;font-size:0.85rem;">Please wait while we set things up.</p></div><style>@keyframes sgspin{to{transform:rotate(360deg)}}</style>';document.body.appendChild(d);window._loadingModal=d}var t=document.getElementById("sgloading-text");if(t)t.textContent=m||"Activating School...";window._loadingModal.style.display="flex"};window._hideLoading=function(){if(window._loadingModal)window._loadingModal.style.display="none"};(function(){var _f=window.fetch;window.fetch=function(){var a=arguments,u=typeof a[0]==="string"?a[0]:"";var opts=a[1]||{},method=opts.method||"GET";var isSchoolPost=method==="POST"&&u.indexOf("/api/admin/schools")>-1;var isSchoolDelete=method==="DELETE"&&u.indexOf("/api/admin/schools")>-1;if(isSchoolPost)window._showLoading("Activating School...");return _f.apply(this,a).then(function(r){if(isSchoolPost){window._hideLoading();r.clone().json().then(function(d){if(d&&d.success&&d.school_id){setTimeout(function(){var s=document.getElementById("schoolIdInput");if(s)s.value=d.school_id;if(window._reloadSchools)window._reloadSchools()},200)}else if(d&&!d.success&&d.message){var msg=document.getElementById("manageSchoolsMessage")||document.getElementById("licenseMessage");if(msg){msg.textContent=d.message;msg.className="form-message error"}}}).catch(function(){})}if(isSchoolDelete){r.clone().json().then(function(d){if(d&&d.success&&window._reloadSchools){setTimeout(window._reloadSchools,500)}}).catch(function(){})}return r}).catch(function(e){if(isSchoolPost){window._hideLoading();var msg=document.getElementById("manageSchoolsMessage")||document.getElementById("licenseMessage");if(msg){msg.textContent="Network error. Please check your connection.";msg.className="form-message error"}}throw e})}})();window._reloadSchools=function(){var tbody=document.getElementById("schoolsTableBody");var sel=document.getElementById("notifTargetSchool");if(!tbody)return;tbody.innerHTML='<tr><td colspan="9" style="text-align:center;padding:16px;">Refreshing...</td></tr>';fetch("/api/admin/schools").then(function(r){if(!r.ok)throw new Error("HTTP "+r.status);return r.json()}).then(function(d){if(!d.success||!Array.isArray(d.schools)){tbody.innerHTML='<tr><td colspan="9" style="text-align:center;padding:16px;color:#d64b4b;">Failed to load schools.</td></tr>';return}var schools=d.schools;if(!schools.length){tbody.innerHTML='<tr><td colspan="9" style="text-align:center;padding:16px;">No schools found.</td></tr>';return}function eh(v){return String(v||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function ea(v){return eh(v).replace(/"/g,"&quot;")}var opts='<option value="">All Schools</option>';tbody.innerHTML=schools.map(function(s){var sc=s.status==="active"&&!s.modules_locked?"active":"inactive";var sl=s.status==="active"&&!s.modules_locked?"Active":"Inactive";var ls=s.last_seen?new Date(s.last_seen).toLocaleString():"-";opts+='<option value="'+ea(s.school_id)+'">'+eh(s.school_name||s.school_id)+"</option>";return'<tr><td style="font-family:monospace;font-size:12px;">'+eh(s.school_id||"-")+'</td><td>'+eh(s.school_name||"-")+'</td><td>'+eh(s.email||"-")+'</td><td>'+eh(s.plan||"-")+'</td><td>'+eh(s.start_date?s.start_date.slice(0,10):"-")+'</td><td>'+eh(s.expiry_date?s.expiry_date.slice(0,10):"-")+'</td><td><span class="status-pill '+sc+'">'+sl+'</span></td><td style="font-size:12px;">'+eh(ls)+'</td><td><button type="button" class="table-action-btn" data-school-action="edit" data-school-id="'+ea(s.school_id)+'" style="font-size:12px;padding:2px 8px;">Edit</button> <button type="button" class="table-action-btn" data-school-action="reset-tokens" data-school-id="'+ea(s.school_id)+'" style="font-size:12px;padding:2px 8px;">Reset Tokens</button> <button type="button" class="table-action-btn" data-school-action="toggle" data-school-id="'+ea(s.school_id)+'" style="font-size:12px;padding:2px 8px;">'+(s.status==="active"&&!s.modules_locked?"Deactivate":"Activate")+'</button> <button type="button" class="table-action-btn danger" data-school-action="delete" data-school-id="'+ea(s.school_id)+'" style="font-size:12px;padding:2px 8px;">Delete</button></td></tr>'}).join("");if(sel){var cv=sel.value;sel.innerHTML=opts;if(cv)sel.value=cv}}).catch(function(){tbody.innerHTML='<tr><td colspan="9" style="text-align:center;padding:16px;color:#d64b4b;">Error loading schools. <a href="#" onclick="location.reload()">Refresh page</a></td></tr>'})}</script>`;
